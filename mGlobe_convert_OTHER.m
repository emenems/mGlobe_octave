...
for i = 1:size(time,1);
    ...
    try
        if file_type <= 2     
            d = dlmread(fullfile(input_path,new_input),'',header_lines,0);                                            % open/read file
            x = unique(d(:,1));                                             % get unique x data
            y = unique(d(:,2));                                             % get unique y data
            z = d(:,3);                                                     % get all z data               
            [out_mat.lat,out_mat.lon] = meshgrid(x,y);                              % create meshgrid
            out_mat.total = out_mat.lon.*0;                                        % create height matrix
            for ii = 1:length(y)                                             % find data for each unique y
                r = find(d(:,2) == y(ii));
                out_mat.total(ii,:) = z(r);
            end
			      out_mat.input_file = new_input;
			      out_mat.units = 'mm';
            clear d x y z ii
        else                                                                % Lat,Lon,value structure
            d = dlmread(fullfile(input_path,new_input),'',header_lines,0);                                            % open/read file
            x = unique(d(:,1));                                             % get unique x data
            y = unique(d(:,2));                                             % get unique y data
            z = d(:,3);                                                     % get all z data                                             

            [out_mat.lon,out_mat.lat] = meshgrid(x,y);                              % create meshgrid
            out_mat.total = out_mat.lon.*0;                                        % create height matrix
            for ii = 1:length(y)                                             % find data for each unique y
                r = find(d(:,2) == y(ii));
                out_mat.total(ii,:) = z(r);
    ...
    if check_out == 0;                                                      % write file if the loading was successful
        ...
        save(out_file_save,'out_mat','-mat7-binary');										% save the created file
    else

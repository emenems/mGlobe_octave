...
%% for loop
%[mem1,mem2] = memory;                                               % futher computation depends on available memory (for Matlab only)
mem2.PhysicalMemory.Available = 1e+8;
for i = 1:length(time(:,7))
    ...
    %     Snm(degree<=1) = 0;                                               % set degree 0 - 1 coefficients to zero
        ...
        %% Write
        save(sprintf('%s%04d%02d%02d_%02d.mat',fullfile(output_path,output_prefix),cyear,cmonth,cday,chour),'omct','-mat7-binary'); % save created grid

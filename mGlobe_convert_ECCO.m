        ...
        if time_resol < 6
            for i = 1:length(time(:,7))
                try
                    ...
                    time_all = double(netcdf_getVar(ncid,5));                      % get time
                    r = find(datenum(1970,1,1,0,0,0)+time_all/24==time(i,7));
                    if ~isempty(r)
                        out_mat.time = time_all(r);
                        obp_all = double(netcdf_getVar(ncid,7));
        ...
        %                 out_mat.lon(out_mat.lon>=180) = out_mat.lon(out_mat.lon>=180)-360;
            ...
            for i = 1:length(time(:,7))
                ...
                try
                    ...
                    save(fullfile(ghc_path,sprintf('ECCO1_M_%04d%02d.mat',time(i,1),time(i,2))),'out_mat','-mat7-binary'); % save model values
                    if size(time,1) > 2
                        out_message = sprintf('Models: converting ECCO model ... (%3.0f%%)',100*((i-1)/size(time,1))); % create status message
                    else
                        out_message = sprintf('Models: converting ECCO model ...'); % create status message
    ...
    case 4
        for i = 1:length(time(:,7))
            try
                ...
                ncid = netcdf_open(file_name,'NC_NOWRITE');                     % open NetCDF file
                out_mat.lat = double(netcdf_getVar(ncid,1));                   % get latitude
                out_mat.time = double(netcdf_getVar(ncid,0));                  % get time
                out_mat.lon = double(netcdf_getVar(ncid,2));                   % get longitude
                out_mat.obp = double(netcdf_getVar(ncid,3));                   % for ECCO2  Bottom Pressure Pot. Anomaly 

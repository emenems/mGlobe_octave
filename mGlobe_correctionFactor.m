...
% This function requires Octave + 8 GB of RAM and installed netcdf package 
% (pkg install -forge netcdf). To reduce the resolution of the computation grid, 
% modify row 59.
% Additionally, this function uses several functions of the mGlobe toolbox
...
%   orography_file  ...     NetCDF (ERA or MERRA) file containing the 
...
%   mGlobe_output = 'CO_MGLOBE_ATMO_ERA_012010_122013_6h.txt'; 
...
resol = 0.0003;                                                             % fixed spatial resolution
...
switch orography_file(end-1:end)
    case 'nc'
        ncid_ref = netcdf_open(orography_file,'NC_NOWRITE');                        % open given netCDF files
        ...
        for nc = 1:nvars
            ...
            switch varname
                ...
                case 'PHIS'
                    orography.data = double(netcdf_getVar(ncid_ref,nc-1));         % get orography
                    try
        ...
        netcdf_close(ncid_ref);                                                       % geopotential to height
end

%% Create grid
disp('Preparing computation...\n');
...
clear orography;                                                        % remove used variables
clear dem;

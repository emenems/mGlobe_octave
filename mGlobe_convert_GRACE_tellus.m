...
try
    ...
    if ~isempty(scale_file)                                                 % load scale matrix-if given
        scale_ncid = netcdf_open(scale_file,'NC_NOWRITE');
        [scale_ndims,scale_nvars] = netcdf_inq(scale_ncid);
        for i = 1:scale_nvars                                               % let user pick the scale matrix
            varname(i) = {netcdf_inqVar(scale_ncid,i-1)};
        end
        [scale_selection,scale_confirm] = listdlg('ListString',varname,'Name','Pick SCALE matrix','ListSize',[round(160*2),round(300*1.1)],"SelectionMode","Multiple"); % pick scale layer
        if scale_confirm == 1 && length(scale_selection) == 1
            temp_scale = double(netcdf_getVar(scale_ncid,scale_selection-1)); % load scale matrix (transpose later)
    ...
    ncid = netcdf_open(file_path,'NC_NOWRITE');                             % open GRACE file
    latitude = double(netcdf_getVar(ncid,1));                              % read latitude
    longitude = double(netcdf_getVar(ncid,0));                             % read longitude
    time = double(netcdf_getVar(ncid,2));                                  % read time info (in days)
    try_var = double(netcdf_getVar(ncid,3));  
    if size(try_var,1) > 3
        time_bounds = double(netcdf_getVar(ncid,4));                       % bounding time
        lwe_thickness = double(netcdf_getVar(ncid,3));                     % water layer in cm
    else
        time_bounds = double(netcdf_getVar(ncid,3));                       % bounding time
        lwe_thickness = double(netcdf_getVar(ncid,4));                     % water layer in cm
    end
    ...
    if ~isempty(rid)
        ...
        for i = 1:length(rid)
            ...
            switch ocean_land
                case 1
                    save(fullfile(ghc_path,'LAND',sprintf('%s_%04d%02d%02d_%02d.mat',output_name,year_temp,month_temp,day_temp,hour_temp)),'grace','-mat7-binary');
                case 2
                    save(fullfile(ghc_path,'OCEAN',sprintf('%s_%04d%02d%02d_%02d.mat',output_name,year_temp,month_temp,day_temp,hour_temp)),'grace','-mat7-binary');
            end

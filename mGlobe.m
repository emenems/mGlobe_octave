...
% mGlobe OCTAVE toolbox allows user to calculate the global hydrological, 
...
% 
% System requirements:
%   OCTAVE 4.0.0 GNU (GUI for Windows only)
...
%% GUI
        ...
        uicontrol(F1,'Position',[9,1,570,26],'Style','Text',...
            'FontSize',8,'FontAngle','italic','String','Set the global hydrological effect',...
            'Tag','text_status');
        uicontrol(F1,'Position',[580,6,55,19],'Style','Pushbutton',...      % Help button: no callbak
        ...
        uicontrol(p1,'Style','Text','Units','characters',...
            'position',[65.4 1.846 16.6 1.075],...
            'String','Threshold (deg):');
        uicontrol(p1,'Style','Edit','Units','characters',...
            'position',[83.4 1.538 8.2 1.692],'BackgroundColor','white',...
        ...
        uicontrol(p5,'Style','Text','Units','characters',...
            'position',[65.5 1.846 16.6 1.075],...
            'String','Threshold (deg):');
        ...
        uicontrol(p1_5,'units','characters','Position',[13.6 2.769 27.6 1.692],...
                    ...
                    'String','GLDAS/CLM|GLDAS/MOS|GLDAS/NOAH (0.25)|GLDAS/NOAH (1deg)|GLDAS/VIC|ERA Interim|MERRA (assimilation)|Other|GRACE|NCEP Reanalysis-2|MERRA2|NCEP Reanalysis-1|GLDASv2.1/NOAH (0.25)',...
        ...
        uicontrol(p2_1,'units','characters','Position',[91 14.154 11.2 1.692],...
                    'Style','Popupmenu','String','txt|mat|tsf',...
        ...
        uicontrol(p2_2,'units','characters','Position',[63 2.769,10.4,1.154],...
                    'Style','Text','String','Max val.');
        uicontrol(p2_2,'units','characters','Position',[47.4 2.769,10.4,1.154],...
                    'Style','Text','String','File type');
        ...
        uicontrol(p2_2,'units','characters','Position',[1.2 0.923,15,1.769],...
                    'Style','Pushbutton','String','Select grid',...
        ...
        uicontrol(p2_2,'units','characters','Position',[47.0 1 15.2 1.692],...
                    'Style','Popupmenu','String','txt (Lon,Lat,d)|txt (Lat,Lon,d)|mat|arc ASCII|grd 6 text|netCDF',...
        ...
        uicontrol(p2_2,'units','characters','Position',[16.8,1,28,1.692],...
                    'Style','Popupmenu','String','All included|Exclude Greenland|Exclude Antarctica|Exc. Greenland+Antarc.|Inlclusion polygon ',...
                    'BackgroundColor','white',...
                    'Tag','popup_view_exclude','Value',1);
        uicontrol(p2_2,'units','characters','Position',[63.4,1,9.8,1.692],...
                    'Style','Edit','String','1000','BackgroundColor','white',...
                    'Tag','edit_view_data_max');
        uicontrol(p2_2,'units','characters','Position',[75.1 1 13 1.692],...
                    'Style','Popupmenu','String','Save As|eps|png','BackgroundColor','white',...
        ...
        uicontrol(p3_3,'units','characters','Position',[10.4 0.6 13.6+8 1.692],...
                    'Style','Popupmenu','Value',1,'String',...
                    'Lat,Lon,H (m)|Lat,Lon,H (mm)|Lon,Lat,H (m)|Lon,Lat,H (mm)',...
        ...
        uicontrol(p3_5,'units','characters','Position',[11.8 1.11 10.6 1.08],...
                    'Style','Text','String','file type:',...
                    'Tag','text_down_dem_input');
        uicontrol(p3_5,'units','characters','Position',[48 0.6 8.4 1.692],...
                    'Style','Pushbutton','String','Output','UserData',[],...
        ...
        uicontrol(p3_5,'units','characters','Position',[81 0.78 21.4 1.7],...
                    'Style','Pushbutton','String','Convert DEM',...
                    'Tag','push_down_dem_run','CallBack','mGlobe calc_down_dem');
        uicontrol(p3_5,'units','characters','Position',[23 0.6 23 1.69],...
        ...
        uicontrol(p5_3,'units','characters','Position',[78.5,0.238+3.8,8,1.769],...
                    'Style','Text','units','characters',...
                    'String','Paht:','UserData',fullfile('OBPM'),...
                    'Tag','push_ocean_convert_output');
        uicontrol(p5_3,'units','characters','Position',[87,0.238+3.8,28,1.769],...
        ...
        uicontrol(p5_3,'units','characters','Position',[18,0.238+4,60,1.769],...
                    'Style','Popupmenu','units','characters','BackgroundColor','white',...
                    'String','ECCO-JPL (convert: *.txt for monthly and *.cdf for 12 hours)|Other (calculate only)|GRACE (calculate only)|ECCO2_beta (conversion: *.nc)|OMCT_oba (convert: *.txt AOD1B spherical harmonics)|OMCT_ocn (convert: *.txt AOD1B spherical harmonics)|OMCT_atm (convert: *.txt AOD1B spherical harmonics)','UserData',[],...
                    'Tag','popup_ocean_load_convert_model','CallBack','mGlobe popup_ocean_model');
        uicontrol(p5_3,'units','characters','Position',[18,2.03,60,1.769],...
                    'Style','Popupmenu','units','characters','BackgroundColor','white',...
        ...
        switch in_switch                                                    % switch between callbacks  
                ...
                if isempty(dem_path)                                        % check if a DEM has been selected
                    ...
                    try                                                     % try to display the DEM
                        ...
                        if numel(dem.lon) > 1000*1000 && numel(dem.lon) <= 2000*2000                    % reduce the size of plotted matrix
                          ...
                          dem.height = dem.height(1:2:end,1:2:end);
                        elseif numel(dem.lon) > 2000*2000
                          dem.lon = dem.lon(1:4:end,1:4:end);
                          dem.lat = dem.lat(1:4:end,1:4:end);
                          dem.height = dem.height(1:4:end,1:4:end);
                        end
                        surf(dem.lon,dem.lat,dem.height,'EdgeColor','none'); 
                        hold on
                        plot3(Input(1),Input(2),Input(3),'k.','MarkerSize',18);
                        caxis([min(min(dem.height)),max(max(dem.height))]);
            ...
            case 'hydro_calc'                                              % Calculate the Continental Water Storage Effect
                ...
                if model_calc == 9                                          % prompt user to pick GRACE model (prefix will be determined)
                    ...
                    [name,path] = uigetfile(fullfile(ghc_path),'Pick file with *.mat GRACE grid, first 22 letters will be used as a PREFIX (e.g. GRC_GFZ_RL05_FILT0_CON)');
                    if name ~=0                                             % some file name is expected
            ...
            case 'view_2Dplot'                                              % 2D plot (surfm)
                ...
                map_extend = 7;%get(findobj('Tag','popup_view_mapextend'),'Value');
            ...
            case 'calc_down_gldas'
                [name,path] = uigetfile({'*.nc;*.nc4','NetCDF (*.nc,*.nc4)';...
                            '*.*','All files'},'Choose ONE input GLDAS or MERRA netCDF file'); % select MERRA/GLDAS interim netcdf (*.nc) input file
                if name == 0                                                
                    ...
                    if start_calc > end_calc                                    % check valid settings and perform conversion
                        ...
                        if model_version <=8 
                            mGlobe_convert_GLDAS(start_calc,end_calc,model_version,step_calc,output_path,input_path,input_file); % convert GLDAS/MERRA data
                        end
            ...
            case 'ocean_calc'                                              % Calculate the non-tidal ocean loading effect (NTOL)
                ...
                case 'load_atmo_orography'                                  % load netCDF ERA orography data
                    [name,path] = uigetfile('*.*','Orography, ERA: netcdf (*.nc), MERRA: netcdf (*.nc)');
                    if name == 0                                            % If cancelled-> no data
            ...
            case 'atmo_calc_era'                                                % Calculate atmospheric effect
                ...
                try                                                         % check if every file is OK
                    ncid_ref = netcdf_open(file_ref,'NC_NOWRITE');          % try to open all files (check)
                    netcdf_close(ncid_ref);
                    ncid_ref = netcdf_open(file_temp,'NC_NOWRITE');
                    netcdf_close(ncid_ref);
                    ncid_ref= netcdf_open(file_humid,'NC_NOWRITE');
                    netcdf_close(ncid_ref);
                    ncid_ref = netcdf_open(file_height,'NC_NOWRITE');
                    netcdf_close(ncid_ref);
                    ncid_ref = netcdf_open(file_sp,'NC_NOWRITE');
                    netcdf_close(ncid_ref);
                catch
                    check_load = 1;
                end
            ...
            case 'atmo_calc_merra'                                          % Calculate MERRA atmospheric effect
                ...
                try                                                         % check if every file is OK
                    ncid_ref = netcdf_open(file_ref,'NC_NOWRITE');         % try to open all files (check), orography in hdf file, all others in netcdf file format
                    netcdf_close(ncid_ref);
                    ncid_ref = netcdf_open(file_temp,'NC_NOWRITE');
                    netcdf_close(ncid_ref);
                    ncid_ref= netcdf_open(file_humid,'NC_NOWRITE');
                    ...
                    ncid_ref = netcdf_open(file_sp,'NC_NOWRITE');
                    netcdf_close(ncid_ref);
                    ncid_ref = netcdf_open(file_sthd,'NC_NOWRITE');
                    netcdf_close(ncid_ref);
                catch
                    check_load = 1;
                end

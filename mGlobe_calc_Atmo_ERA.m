...
netcdf_close(ncid_ref);                                                     % close the netcdf file
...
for i = 1:size(time,1)
    ...
    try
        ncid_temp = netcdf_open([file_temp(1:end-10),sprintf('%02d',time(i,4)),sprintf('_%04d.nc',time(i,1))],'NC_NOWRITE'); % open temperature file
        ncid_humid = netcdf_open([file_humid(1:end-10),sprintf('%02d',time(i,4)),sprintf('_%04d.nc',time(i,1))],'NC_NOWRITE'); % open humidity file
        ncid_height = netcdf_open([file_height(1:end-10),sprintf('%02d',time(i,4)),sprintf('_%04d.nc',time(i,1))],'NC_NOWRITE'); % open geopotential file
        ncid_sp = netcdf_open([file_sp(1:end-9),'D',sprintf('_%04d.nc',time(i,1))],'NC_NOWRITE'); % open surface data file
    catch
    ...
    for nc = 1:nvars
        ...
        switch varname
            ...
            case 'levelist'
                temperature.level = double(netcdf_getVar(ncid_temp,nc-1)); % info on ERA pressure levels = old ERA data server
            case 'level'
                temperature.level = double(netcdf_getVar(ncid_temp,nc-1)); % info on ERA pressure levels = new ERA data server
            case 't'
                ...
                if ~isempty(rid)
                    ...
                    try
                        scale_factor = netcdf_getAtt(ncid_temp,nc-1,'scale_factor'); % try to get the scaling factor
                    catch
                        scale_factor = 1;
                    end
                    try
                        add_offset = netcdf_getAtt(ncid_temp,nc-1,'add_offset'); % try to get the adding factor
                    catch
                        add_offset = 0;
                    end
    ...
    netcdf_close(ncid_temp);                                                % close the temperature file
        ...
        switch varname
            ...
            case 'q'
                ...
                if ~isempty(rid)
                    ...
                    try
                        scale_factor = netcdf_getAtt(ncid_humid,nc-1,'scale_factor');
                    catch
                        scale_factor = 1;
                    end
                    try
                        add_offset = netcdf_getAtt(ncid_humid,nc-1,'add_offset');
                    catch
                        add_offset = 0;
                    end
        ...
        switch varname
            ...
            case 'z'
                ...
                if ~isempty(rid)
                    ...
                    try
                        scale_factor = netcdf_getAtt(ncid_height,nc-1,'scale_factor');
                    catch
                        scale_factor = 1;
                    end
                    try
                        add_offset = netcdf_getAtt(ncid_height,nc-1,'add_offset');
                    catch
                        add_offset = 0;
                    end
        ...
        switch varname
            ...
            case 'sp'                                                       % surface pressure
                ...
                if ~isempty(rid)
                    ...
                    try
                        scale_factor = netcdf_getAtt(ncid_sp,nc-1,'scale_factor');
                    catch
                        scale_factor = 1;
                    end
                    try
                        add_offset = netcdf_getAtt(ncid_sp,nc-1,'add_offset');
                    catch
                        add_offset = 0;
                    end
            ...
            case 't2m'                                                      % 2 m temperature
                rid = find(surface.time == (time(i,7) - time_ref)*24);
                if ~isempty(rid)
                    surface.temperature = double(netcdf_getVar(ncid_sp,nc-1,[0 0 rid-1],[length(surface.lon) length(surface.lat) 1]));
                    try
                        scale_factor = netcdf_getAtt(ncid_sp,nc-1,'scale_factor');
                    catch
                        scale_factor = 1;
            ...
            case 'd2m'                                                      % 2 m dewpoint (humidity)
                rid = find(surface.time == (time(i,7) - time_ref)*24);
                if ~isempty(rid)
                    surface.dewpoint = double(netcdf_getVar(ncid_sp,nc-1,[0 0 rid-1],[length(surface.lon) length(surface.lat) 1]));
                    try
                        scale_factor = netcdf_getAtt(ncid_sp,nc-1,'scale_factor');
                    catch
                        scale_factor = 1;
